...
export const LoadingScreen: React.FC<LoadingScreenProps> = ({ onComplete }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-[#0a0a0f] flex flex-col justify-between p-8 md:p-12 lg:p-16 select-none overflow-hidden font-mono">
      {/* Background dot grid for the loading screen */}
      ...
      <div className="flex items-start relative z-10">
        <motion.div
          ...
          className="text-xs text-muted uppercase tracking-[0.3em] font-semibold flex items-center gap-2"
        >
          <span className="text-accent">//</span> PORTFOLIO BUILD SYSTEM © 2026
        </motion.div>
      </div>
      ...
      <div className="flex justify-center items-center my-auto relative z-10">
        <AnimatePresence mode="wait">
          <motion.h2
            ...
            className="text-4xl md:text-6xl lg:text-7xl font-bold tracking-wider text-accent select-none pointer-events-none"
          >
            {words[wordIndex]}
          </motion.h2>
        </AnimatePresence>
      ...
      <div className="w-full flex flex-col md:flex-row justify-between items-end gap-6 pt-4 relative z-10">
        ...
        <div className="flex flex-col items-end">
          <span className="text-7xl md:text-9xl lg:text-[10rem] font-bold text-white leading-none tabular-nums select-none tracking-tighter">
      ...
      <div className="absolute bottom-0 left-0 right-0 h-[4px] bg-white/5 overflow-hidden z-20">
        <div
          className="h-full bg-accent transition-transform duration-75 ease-out origin-left"
          style={{
